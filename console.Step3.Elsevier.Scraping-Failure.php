...
/*
	...
	php console.Step3.Elsevier.Scraping-Failure.php
	
	OBSERVATION/RESULT:
	DOI gets resolved + we get Publishers homepage for that publication, there it's possible
	to grab the PDF link which actually points on a redriction HTML/Javascript page.
...
//		if ( $pdfCount != 1 ) {	echo $pid . " ::: PDFs: " . $pdfCount . " ::: " . $doi . " ::: Page-Range: " . $doraPageSum . "\r\n"; }
	...
	foreach( ['mods_relatedItem_host_originInfo_publisher_mt','mods_originInfo_publisher_mt'] as $field ) {
		...
		foreach( $pubAry[$field] as $tmp ) { 
	//		if ( $idx = strtolower($tmp) ) {
			if ( $idx = trim($tmp) ) {
				$pblrByPubAry[$idx] = $tmp;
				$pxFoundAry[$idx] = ( @empty($pxFoundAry[$idx]) ? $pubAry['PID'] : strval(max(@intval($pxFoundAry[$idx]),1) + 1 ).'*' );
			}
		}
	}
...
//	https://pubs.rsc.org/en/content/articlepdf/2010/em/c002020f
	...
	echo "PDF-URL found: " . ( empty($pdfUrl) ? '-none-' : $pdfUrl ) . "\r\n";
	exit; // TEST
...
/*	
ksort($pxFoundAry);
echo print_r($pxFoundAry,1) . "\r\n";
echo print_r($solrFieldAry,1) . "\r\n";
echo print_r($pblrFilterAry,1) . "\r\n";
echo "Total Publications: " . sizeof($pidAry['response']['docs']) . ", Publishers found: " . $pxFound . "\r\n";	
*/
echo "Total Publications: " . sizeof($pidAry['response']['docs']) . ", PubliShers found: " . $pxFound . "\r\n";	
