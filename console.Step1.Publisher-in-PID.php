...
/*
	TASK/DESCRIPTION:
	Will find the publication (in a Solr-query-defined set of publications)
	where a given/hard-coded publisher (see $pxWanted) appears the first time.
	This is a pure work file, single tasks were extracted (afterwards)
	into separate test...php files - do not use this, or only with care!

	BASH TEST/WORK COMMANDS:
	php console.Step1.Publisher-in-PID.php
...
function doraPageSum( $pAry ) {
	// see: https://www.wiki.lib4ri.ch/display/TD/Pages + https://www.wiki.lib4ri.ch/pages/viewpage.action?pageId=21889177
	$ary = array( @strval($pAry['mods_part_extent_start_mt'][0]), @strval($pAry['mods_relatedItem_host_part_extent_start_mt'][0]) );
	...
	if ( $sum = max( @intval($pAry['mods_part_extent_end_mt'][0]) - @intval($pAry['mods_part_extent_start_mt'][0]), 0 ) ) {
		return ( $sum + 1 );		// page range 25-27 would be the 3 pages (= diffrence + 1)
	}
	if ( $sum = max( @intval($pAry['mods_relatedItem_host_part_extent_end_mt'][0]) - @intval($pAry['mods_relatedItem_host_part_extent_start_mt'][0]), 0 ) ) {
		return ( $sum + 1 );		// page range 25-27 would be the 3 pages (= diffrence + 1)
	}
	return 0;
}
...
foreach( $pxFilterAry as $task => $pAry ) { // set identical keys:
	$pxFilterAry[$task] = array_combine( array_map('strtolower',$pAry), $pAry );
}
...
/*
	echo 'https://www.dora.lib4ri.ch/islandora/object/' . $pAry['PID'] . "\r\n";
	echo print_r( $pAry, 1 ) . "\r\n";
*/

	// get Publisher:
	if ( !( $pxByPub = @trim($pAry['mods_relatedItem_host_originInfo_publisher_mt'][0]) ) ) {
		if ( !( $pxByPub = @trim($pAry['mods_originInfo_publisher_mt'][0]) ) ) {
			die( "ERROR: No publisher!?\r\n".print_r($pAry,1)."\r\n" );
		}
	}

	$pxByPub = reset( $pAry['mods_relatedItem_host_originInfo_publisher_mt'] );
	if ( empty($pxByPub) ) { continue; }
	$pxCountAry[$pxByPub] = @intval($pxCountAry[$pxByPub]) + 1;

	if ( @!isset($pxFilterAry['unCovered'][strtolower($pxByPub)]) && @!isset($pxFilterAry['toDeCover'][strtolower($pxByPub)]) ) {
		continue;
		die( "ERROR: No *known* publisher!?\r\n".print_r($pAry,1)."\r\n" );
	}
	if ( @!intval($pAry['mods_identifier_doi_mt']) ) {
		continue;
		die( "ERROR: No DOI !?\r\n".print_r($pAry,1)."\r\n" );
	}
	...
	$pxWanted = 'American Chemical Society';

	echo "Publisher by Publication: " . $pxByPub . "\r\n";
	...
	$doi = reset($pAry['mods_identifier_doi_mt']);
	if ( !intval($doi) && !( $doi = strchr($doi,'10.') ) ) { // should there be a leading http:...
		die( "ERROR: No DOI !\r\n".print_r($pAry,1)."\r\n" );
	}

	$doraPageSum = doraPageSum($pAry);
	...
	if ( $_pdf_counting ) {
		for($p=0;$p<10;$p++) {
			...
			if ( @is_array($pAry[$field]) && @intval($pAry[$field][0]) ) { $pdfCount++; }
		}
//		if ( $pdfCount != 1 ) {	echo $pid . " ::: PDFs: " . $pdfCount . " ::: " . $doi . " ::: Page-Range: " . $doraPageSum . "\r\n"; }
	...
	foreach( ['mods_relatedItem_host_originInfo_publisher_mt','mods_originInfo_publisher_mt'] as $field ) {
		if ( @!isset($pAry[$field]) ) { continue; }
		$solrFieldAry[$field] = ( @intval($solrFieldAry[$field]) + 1 );
		foreach( $pAry[$field] as $tmp ) { 
	//		if ( $idx = strtolower($tmp) ) {
			if ( $idx = trim($tmp) ) {
				$pblrByPubAry[$idx] = $tmp;
				$publerFoundAry[$idx] = ( @empty($publerFoundAry[$idx]) ? $pAry['PID'] : strval(max(@intval($publerFoundAry[$idx]),1) + 1 ).'*' );
			}
		}
	}
...
/*
//	$html = $pdfDL->setDoi($doi)->fetchHtml();
	$html = doraMakeCurlQuery( $url, 60, array(), $infoCurlAry );

	if ( $posZ = strpos($html,'.pdf') ) {
		$posA = @intval( strrpos(substr($html,0,$posZ),'=') );
		$html = substr($html,$posA,$posZ-$posA+4);
	} else {
		$html = "??????????????????????????";
	}
*/
...
/*	
ksort($publerFoundAry);
echo print_r($publerFoundAry,1) . "\r\n";
echo print_r($solrFieldAry,1) . "\r\n";
echo print_r($pblrFilterAry,1) . "\r\n";
echo "Total Publications: " . sizeof($pidAry['response']['docs']) . ", Publishers found: " . $publerFound . "\r\n";	
*/
echo "Total Publications: " . sizeof($pidAry['response']['docs']) . ", PubliShers found: " . $publerFound . "\r\n";	
echo "Total: " . print_r( $pxCountAry,1) . "\r\n";	
